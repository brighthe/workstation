...
本目录管理 `dut-postdoc`、`dut-institute-work` 和 `heliangos` 到 iCloud Obsidian 容器的每日镜像同步。执行使用 Windows Task Scheduler，不依赖 Codex、Claude Code 或其他 AI 工具。
...
2. 在新电脑登录 iCloud for Windows，并准备三个仓库。
