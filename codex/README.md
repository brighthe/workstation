...
- 在新的非简单会话或任务开始时，先简短建议任务应保持默认交互、使用 Plan mode，还是按 Goal workflow 运行。
- 如果推荐的工作流需要 UI 切换，而我不能直接替你切换，就请你先切换后再继续。
- 问答、解释、只读检查和小澄清使用默认交互；本地文档将其称为普通模式。
...
- 将我提出的方案或方法视为需要评估的提案，而不是自动接受的内容。采用前，评估其正确性、可行性、关键假设、风险、取舍和替代方案。
- 如果提案不正确、风险不合理，或明显劣于其他选择，应给出具体理由，明确指出问题，并在继续前推荐更好的方案。
